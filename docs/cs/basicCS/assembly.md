...
指的是一系列复杂指令集计算机（Complex Instruction Set Computer, CISC）的指令架构，起源于英特尔基于8086微处理器及其**8位外部总线**的变体8088开发。

其将一系列诸如存、取、计算操作，集于单一指令，这节减了对指令的编写，但因此CISC的指令数目多而复杂，每个指令的字长不等，所付出的代价就是性能和功耗。

但其兼容性很强，如今的Windows PC中一般都会包含有两个重要文件夹<u>Program Files</u>和<u>Program Files（x86）</u>，前者是存放64-bit的系统程序，后者则是32-bit，也就意味着机器能向下兼容以往的程序和库。

###### ARM & RISC-V

指的是以精简指令集计算机（Reduced Instruction Set Computer, RISC）的指令架构，RISC起源自加州伯克利分校，典型代表就是ARM和RISC-V，ARM起源自Acorn设计的处理器Acorn RISC Machine，而RISC-V则于2010年在加州大学伯克利分校启动，V指第五代（亦指Variation）

其设计方案十分简约，通常只有20多条指令的简化指令集，每条指令长度固定，有专用的用于访存的指令，减少了访存时间，大多数运算指令只能访问寄存器，CPU则配有大量寄存器。这些特性使得CPU执行效率高效，并且功耗往往非常低，因此常用于嵌入式开发。
...
![逻辑存储器](/imgs/image-20230425223756909.jpg)
...
Apple 官网[Writing ARM64 code for Apple platforms](https://developer.apple.com/documentation/xcode/writing-arm64-code-for-apple-platforms)

以及[在 Apple Silicon Mac **(ARM64)** 上入门汇编语言](https://evian-zhang.github.io/learn-assembly-on-Apple-Silicon-Mac/index.html)、[macOS **(amd64)** 上的汇编入门](https://github.com/Evian-Zhang/Assembly-on-macOS)
