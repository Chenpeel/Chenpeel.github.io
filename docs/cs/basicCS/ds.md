...
> 数据是信息的载体，是描述客观事物属性的数、字符以及所有能被输入到计算机中并被程序识别处理的符号的集合

##### 数据元素

> 数据的基本单位，可由若干数据项构成，数据项是构成数据元素的最小不可分割单位。
...
> 对特定问题求解步骤的一种描述，是指令的有限序列，每条指令表示一个或一系列操作。
...
- 多个循环体
  依靠两个运算规则
  - 加法规则：$T(n) = T_1(n)+T_2(n)=O\big(f(n)\big)+O\big(g(n)\big) = O\bigg(\max\big(f(n),g(n)\big)\bigg)$
  - 乘法规则：$T(n) = T_1(n)\times T_2(n)=O\big(f(n)\big) \times O\big(g(n)\big)= O\big(f(n) \times g(n)\big)$
...
##### [顺序表](https://github.com/Chenpeel/Codes/tree/master/Cpp/algo_codes/struct/liner_list/sequeue_list.hpp)

> 线性表的顺序存储，用一组地址连续的存储单元，依次存储线性表中的数据元素，从而使得逻
> 辑上相邻的两个元素在物理位置上也相邻。
...
- 存储连续，物理位置顺序=逻辑位置顺序，信息相对密集，密度高
- 无需大量操作即可完成插入和删除

> 缺点

- 元素插入删除需要移动大量元素（由于物理位置也必须连续）
- 必须占据连续的一段存储空间，不够灵活
...
* [递归](/cs/basicCS/algorithms/analysis/recursion.md) ， 程序调用的本质是栈
