...
C语言更多的面向硬件，其为硬件设备封装了一系列库。而C++则补充和扩展了面向对象等特点，是一种更高层次的抽象。

这可以为学习C和C++提供指导性，注重理解C语言的基本概念、库，着重理解C++在其上做的封装和抽象特性。同样在理解封装和抽象时，可以借助其更底层的原理以更深入。
